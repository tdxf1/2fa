...
async function handleOAuthAuthorize(request, env) {
    ...
    try {
        ...
        const authUrl = `${env.OAUTH_BASE_URL}/authorize?${params}`;
...
async function processOAuthCode(code, state, clientIP, request, env, corsHeaders) {
    try {
        ...
        // 验证用户ID
        if (!userData.sub || String(userData.sub) !== String(env.OAUTH_ID)) {
            await recordOAuthAttempt(clientIP, false);
        ...
        const payload = {
            userInfo: {
                ...
                avatar_template: userData.picture
            },
            ip: clientIP,
            loginMethod: 'oauth',
            iat: Math.floor(Date.now() / 1000),
    ...
    <div class="container">
		...
		<button onclick="startOAuthLogin()" class="oauth-login-btn">
		    <span class="oauth-icon">
		        <img src="https://raw.githubusercontent.com/tdxf1/2fa/refs/heads/main/auth0.svg"
		             alt="Logo" 
		             style="width: 40px; height: 40px; object-fit: contain;">
		    </span>
		    <span>使用 Auth0 号登录</span>
		</button>
